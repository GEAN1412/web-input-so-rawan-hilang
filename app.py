...
def load_user_save(toko_id, master_version):
    """Memuat simpanan toko dari folder versi master saat ini"""
    try:
        # Path: so_rawan_hilang/rekap/{master_version}/Hasil_{toko_id}.xlsx
            ...
            with st.spinner("Menarik data..."):
                ...
                if m_df is not None:
                    try:
                        ...
                        m_tk = m_df.columns[0]
                        # Hanya tarik rekap dari FOLDER VERSI saat ini
...
# ==========================================
    ...
    ti = st.text_input("📍 Masukkan Kode Toko (4 Digit):", max_chars=4).upper()
    ...
    if st.button("🔍 Cari Data") or st.session_state.toko_cari:
        if ti:
            ...
            if df_master is not None:
                # 2. CARI SIMPANAN TOKO DI FOLDER VERSI TERSEBUT
                df_user = load_user_save(st.session_state.toko_cari, m_ver)
                
                # Filter master untuk toko ini (fallback)
                ...
                if df_user is not None:
                    data_show = df_user
                    st.success("📝 Melanjutkan inputan Anda (Versi Master Terbaru).")
                else:
                ...
                if not data_show.empty:
                    ...
                    edited = st.data_editor(
                        ...
                        hide_index=True, use_container_width=True, key=f"ed_v7_{st.session_state.toko_cari}"
                    )
