...
@st.cache_data(ttl=10) # Cache singkat untuk menghemat API call
def get_master_info():
    try:
...
def load_user_save(toko_id, v_id):
    try:
        p_id = f"so_rawan_hilang/hasil/v{v_id}/Hasil_{toko_id}.xlsx"
...
def confirm_admin_publish(file_obj):
    st.warning("Anda akan Publish Master baru. Tampilan toko akan otomatis ter-reset.")
    st.info("Catatan: Data lama akan diabaikan oleh sistem untuk menghemat kuota API.")
    if st.button("IYA, Publish Sekarang", type="primary", use_container_width=True):
        try:
            # FIX: KITA HAPUS PERINTAH DELETE MASSAL UNTUK MENGHINDARI RATE LIMIT 420
            ...
            st.success("✅ Master Terbit! Selesai."); time.sleep(2); st.rerun()
...
def confirm_user_submit(data_toko, toko_code, v_id):
    if st.button("Ya, Simpan ke Cloud", use_container_width=True):
        ...
        try:
            ...
            p_id = f"so_rawan_hilang/hasil/v{v_id}/Hasil_{toko_code}.xlsx"
...
# ==========================================
    ...
    if not st.session_state.admin_auth:
        ...
        with tab1:
            ...
            if f_adm and st.button("🚀 Publish Master Baru"): confirm_admin_publish(f_adm)
            st.divider()
            m_df, m_ver = get_master_info()
            if m_df is not None:
                res = cloudinary.api.resources(resource_type="raw", type="upload", prefix=f"so_rawan_hilang/hasil/v{m_ver}/", max_results=500)
                submitted_files = res.get('resources', [])
                st.info(f"📊 Terdapat **{len(submitted_files)}** toko yang sudah mengirim laporan.")
                if st.button("🔄 Gabung Data Seluruh Toko", use_container_width=True):
                    ...
                    st.download_button(f"📥 Download Rekap", buf.getvalue(), f"Rekap_{get_indonesia_date()}.xlsx", use_container_width=True)
        ...
        with tab3:
            t_nik = st.text_input("NIK reset:"); n_pw = st.text_input("Pass Baru:", type="password")
            if st.button("Simpan Password Baru"):
                db = load_json_db(USER_DB_PATH)
                if t_nik in db: 
                    db[t_nik] = n_pw; save_json_db(USER_DB_PATH, db); st.success("✅ Berhasil!")
...
# ==========================================
    ...
    if st.button("🔍 Cari Data") or st.session_state.user_search_active:
        if t_id:
